...
from ...protocols.endorse_transaction.v1_0.transaction_record import TransactionRecord
...
class TranIdMatchInfoSchema(OpenAPISchema):
    ...
    tran_id = fields.Str(
        description="For writing endorsed transaction to the ledger", required=False, example=UUIDFour.EXAMPLE
    )
...
async def credential_definitions_send_credential_definition(request: web.BaseRequest):
    ...
    transaction_record = await TransactionRecord.retrieve_by_id(context, transaction_id)

    if transaction_record.state == "endorse":

        rev_reg_size = transaction_record.messages_attach[0]["data"]["json"]["operation"]["data"]["revocation_registry_size"]
        schema_id = transaction_record.messages_attach[0]["data"]["json"]["operation"]["data"]["schema_id"]
        support_revocation = transaction_record.messages_attach[0]["data"]["json"]["operation"]["data"]["support_revocation"]
        tag = transaction_record.messages_attach[0]["data"]["json"]["operation"]["data"]["tag"]
        ...
        return web.json_response({"error":"Cannot write transaction to ledger, as it is not endorsed"})
