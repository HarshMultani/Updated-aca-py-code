...
class ConnectionManager:
    ...
    async def create_request(
        ...
        if my_role is not None:
            connection.messages_attach["data"]["json"]["tx_my_role"].append(my_role)
    ...
    async def receive_request(
        ...
        if connection:
            ...
            if request.my_role is not None:
                connection.messages_attach["data"]["json"]["tx_their_role"].append(request.my_role)
            connection.state = ConnRecord.State.REQUEST.rfc160
            await connection.save(
    ...
    async def accept_response(
        ...
        if response.my_role is not None:
            connection.messages_attach["data"]["json"]["tx_their_role"].append(response.my_role)
        connection.state = ConnRecord.State.RESPONSE.rfc160
